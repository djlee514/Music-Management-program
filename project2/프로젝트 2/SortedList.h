...
public:
	// =연산자 오버로딩
	void operator =(const SortedList<T>& _p);

	// ==연산자 오버로딩
	bool operator==(SortedList<T> &_p);
	...
	int Add(T& data);	//배열에 Primary key를 기준으로 정렬해 새로운 항목을 추가
...
template<typename T>
void SortedList<T>::operator=(const SortedList<T>& _p)
{
...
template<typename T>
bool SortedList<T>::operator==(SortedList<T>& _p)
{
	// 더 적은길이
	int length = this->GetLength() < _p.GetLength() ? this->GetLength() : _p.GetLength();
	this->ResetList();  // this cur 초기화
	_p.ResetList();    // temp cur 초기화
	T item1, item2;
	int count = 1;      // count까지만 비교
	// this item과 temp item 비교
	while (this->GetNextItem(item1) != -1 && _p.GetNextItem(item2) != -1) {
		if (item1 != item2) {
			return 0;  // length전에 같지않으면 리턴 0
		}
		if (count == length) {
			return 1;  // length까지 같으면 리턴 1
		}
		count++;
	}
}
...
int SortedList<T>::RetrieveMusic_Binary(T& data)
{
	...
	bool test = false;
	while (first <= last && !test)	//first가 last값보다 작거나 같을 때만 while문 실행
	{
